...
public void execute(Runnable command) {
     ...
     * 或者自从进入此方法后池关* 闭了。 所以我们重新检查状态，如果必要的话，如果没有，则回滚入队，或者如果没有，
     * 则启动新的线程。
     */
...
FixedThreadPool并不是一个类，仅仅是当核心线程数==最大线程数（也就是就只有核心线程），并且任务队列是没有限制的的ThreadPoolExecutor，我们从源代码可以看出LinkedBlockingQueue没有指定大小
```Java
...
直接看源代码，很直接，没有核心线程（常驻的线程），最大线程数为最大整数（2<sup>31</sup>-1.），并且每个非核心线程空闲等待的时间为60s，采用SynchronousQueue队列
```Java
